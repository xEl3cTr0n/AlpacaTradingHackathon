  ...
  return (
    <section className="panel layered-market" aria-labelledby="market-layers-title">
      ...
      <div className="layer-stack">
        ...
        <div className="layer-connector" aria-hidden="true"><span /></div>
        <article className="market-layer micro-layer"><div className="layer-index">03</div><div className="layer-copy"><span>Microstructure of Options Order Dynamics</span><h3>MOOD {mood.mood.toUpperCase()}</h3><strong>VIBE · {mood.vibe.toUpperCase()}</strong><p>{mood.rationale}</p></div><dl><div><dt>Gamma</dt><dd>{micro.gamma_regime}</dd></div><div><dt>GMC</dt><dd>{pct(micro.gamma_concentration)}</dd></div><div><dt>Coverage</dt><dd>{pct(mood.input_coverage)}</dd></div></dl></article>
      </div>
