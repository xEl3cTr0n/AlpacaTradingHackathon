  ...
  return (
    <div className="view-stack">
      ...
      <section className="ops-grid">
        <article className="panel pipeline-panel"><div className="panel-heading"><div><p className="eyebrow">Autonomous decision graph</p><h2>Current run pipeline</h2></div><span className="source-label">{snapshot.market.symbol}</span></div><div className="pipeline-flow">{["API + MCP research", "Regime + rotation", "Swing + GEX", "6-agent vote", "Risk gate", "CLI paper order"].map((node, index) => <div key={node} className="pipeline-node"><span>{index + 1}</span><strong>{node}</strong>{index < 5 && <i aria-hidden="true" />}</div>)}</div><div className="run-summary"><Bot size={19} aria-hidden="true" /><div><strong>{snapshot.strategy.display_name}</strong><p>{snapshot.strategy.thesis}</p></div><span className={snapshot.risk.approved ? "positive" : "negative"}>{snapshot.risk.approved ? "authorized" : "vetoed"}</span></div></article>
