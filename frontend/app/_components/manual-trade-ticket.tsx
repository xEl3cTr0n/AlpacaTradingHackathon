...
export function ManualTradeTicket({ defaultSymbol = "SPY" }: { defaultSymbol?: string }) {
  ...
  function chooseShort(contract: OptionChainContract) {
    ...
    setTrade((current) => ({
      ...
      limit_debit: Math.min(2, Number(nextDebit.toFixed(2))),
    }));
    setPreview(null);
  ...
  return (
    <div className="view-stack">
      <header className="view-heading"><div><p className="eyebrow">Operator console</p><h1>Options chain ticket</h1><p>Browse ten nearby calls or puts, choose two legs, then pass the paper-only risk gate.</p></div><span className="decision-chip approved"><ShieldCheck size={15} aria-hidden="true" /> One spread · $200 cap</span></header>
